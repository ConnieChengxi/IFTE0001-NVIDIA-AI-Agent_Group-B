...
class Valuator:
    ...
    def dcf_valuation(self):
        ...
        with open(rf_path, 'r') as f:
            rf_data = json.load(f)
            risk_free_rate = float(rf_data["data"][0]["value"]) / 100
        ...
        beta = float(self.overview.get("Beta", 1.0))
        ...
        with open(gdp_path, 'r') as f:
            ...
            gdp_vals = [float(x["value"]) for x in gdp_data["data"][:6]]
    ...
    def multiple_valuation(self):
        ...
        nvda_pe = float(self.overview.get("PERatio", 0))
        nvda_eps = float(self.overview.get("EPS", 0))
        
        results = {
            "Tickers": {
                self.symbol: {
                    "MarketCap": float(self.overview.get("MarketCapitalization", 0)),
                    "PE": nvda_pe,
                    "ForwardPE": float(self.overview.get("ForwardPE", 0)),
                    "PB": float(self.overview.get("PriceToBookRatio", 0)),
                    "ROE": float(self.overview.get("ReturnOnEquityTTM", 0)),
                    "EV_EBITDA": float(self.overview.get("EVToEBITDA", 0))
                }
            }
        }
        ...
        for peer in PEERS:
            ...
            if peer_data:
                pe = float(peer_data.get("PERatio", 0))
                # Filter out extreme outliers (e.g., > 500 or <= 0) for a cleaner average
                if 0 < pe < 500:
                    peer_pes.append(pe)
                
                results["Tickers"][peer] = {
                    "MarketCap": float(peer_data.get("MarketCapitalization", 0)),
                    "PE": pe,
                    "ForwardPE": float(peer_data.get("ForwardPE", 0)),
                    "PB": float(peer_data.get("PriceToBookRatio", 0)),
                    "ROE": float(peer_data.get("ReturnOnEquityTTM", 0)),
                    "EV_EBITDA": float(peer_data.get("EVToEBITDA", 0))
                }
    ...
    def ddm_valuation(self, discount_rate, terminal_growth):
        ...
        dps = float(self.overview.get("DividendPerShare", 0))
        if dps == 0:
            return {
                "Intrinsic Price": 0,
