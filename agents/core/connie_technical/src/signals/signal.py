...
def build_signals(df: pd.DataFrame, cfg: SignalConfig = SignalConfig()) -> pd.DataFrame:
    ...
    # -----------------------
    # Route A: enforce exposure floor in bull trend regime
    # -----------------------
    # Start from base_w, then apply floor ONLY when bull_trend is True.
    w = base_w.copy()
    ...
    # -----------------------
    # Soft de-risk scaling (never hard-gate entry)
    # -----------------------
    ...
    # -----------------------
    # Risk exits state machine (for exit signals + diagnostics)
    # -----------------------
