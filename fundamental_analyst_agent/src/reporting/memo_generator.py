...
Write ONLY the description. No preamble."""
    ...
    def _dupont_analysis_section(self) -> str:
        ...
        dupont_3 = dupont['dupont_3_factor']
        
        net_margin = dupont_3.get('net_margin')
        asset_turnover = dupont_3.get('asset_turnover')
        equity_multiplier = dupont_3.get('equity_multiplier')
        actual_roe = dupont_3.get('actual_roe')
        ...
        driver_explanation = dupont.get('driver_explanation', '')
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{"Exceptional" if net_margin and net_margin > 0.20 else "Strong" if net_margin and net_margin > 0.10 else "Moderate" if net_margin and net_margin > 0.05 else "Low"}</td>
                    </tr>
                    <tr>
                        <td><strong>Asset Turnover</strong></td>
                        <td>Revenue / Assets</td>
                        <td class="text-right">{fmt_mult(asset_turnover)}</td>
                        <td>{"High" if asset_turnover and asset_turnover > 1.0 else "Moderate" if asset_turnover and asset_turnover > 0.5 else "Low"}</td>
                    </tr>
                    <tr>
                        <td><strong>Equity Multiplier</strong></td>
                        <td>Assets / Equity</td>
                        <td class="text-right">{fmt_mult(equity_multiplier)}</td>
                        <td>{"High leverage" if equity_multiplier and equity_multiplier > 3.0 else "Moderate" if equity_multiplier and equity_multiplier > 2.0 else "Conservative"}</td>
                    </tr>
                    <tr class="highlight-row">
                        <td><strong>ROE</strong></td>
                        <td>Margin × Turnover × Multiplier</td>
                        <td class="text-right"><strong>{fmt_pct(actual_roe)}</strong></td>
                        <td>{"Exceptional" if actual_roe and actual_roe > 0.25 else "Strong" if actual_roe and actual_roe > 0.15 else "Average" if actual_roe and actual_roe > 0.10 else "Below average"}</td>
                    </tr>
                </tbody>
            </table>
            
            <p style="margin-top: 0.5em; font-size: 9pt; color: #374151;">
                <strong>Primary Driver:</strong> {driver_explanation if driver_explanation else "ROE driven by combination of factors."}
            </p>
        '''
    ...
    def _page4_valuation(self) -> str:
        ...
        html += f'''
            ...
            <ul style="line-height: 1.8;">
                <li><strong>Stage 1 (High Growth):</strong> {stage1_years} years at {self._pct(stage1_growth)} growth</li>
                <li><strong>Stage 2 (Fade Period):</strong> {stage2_years} years, declining to {self._pct(stage2_end_growth)}</li>
                <li><strong>Terminal Growth:</strong> {self._pct(terminal_growth)}</li>
                <li><strong>Total Projection Period:</strong> {total_years} years</li>
                <li><strong>WACC:</strong> {self._pct(wacc)} (Cost of Equity: {self._pct(risk_free + beta * erp)}, Beta: {beta:.2f})</li>
            </ul>
            ...
            <ul style="line-height: 1.8;">
                ...
                <li><strong>Metrics:</strong> PEG, P/B, EV/EBITDA</li>
    ...
    def _get_peg_details(self) -> str:
        ...
        company_growth = company_multiples.get('revenue_growth')
        company_peg = company_multiples.get('peg')
        peer_avg_peg = peer_averages.get('avg_peg')
        peer_avg_growth = peer_averages.get('avg_growth')
        adjusted_pe = peg_analysis.get('growth_adjusted_pe')
        method_used = peg_analysis.get('method_used', 'Simple P/E')
        
        details = []
        
        if company_growth is not None:
            details.append(f'<li><strong>Company Revenue Growth:</strong> {self._pct(company_growth)}</li>')
        
        if peer_avg_growth is not None:
            details.append(f'<li><strong>Peer Avg Growth:</strong> {self._pct(peer_avg_growth)}</li>')
        
        if company_peg is not None:
            details.append(f'<li><strong>Company PEG:</strong> {company_peg:.2f}x</li>')
        
        if peer_avg_peg is not None:
            details.append(f'<li><strong>Peer Avg PEG:</strong> {peer_avg_peg:.2f}x</li>')
        
        if adjusted_pe is not None:
            details.append(f'<li><strong>Growth-Adjusted P/E:</strong> {adjusted_pe:.1f}x</li>')
        
        details.append(f'<li><strong>Method:</strong> {method_used}</li>')
        
        if company_peg is not None and peer_avg_peg is not None:
            if company_peg < peer_avg_peg:
                details.append('<li><strong>Note:</strong> Company PEG below peer average suggests undervaluation relative to growth</li>')
            else:
                details.append('<li><strong>Note:</strong> Company PEG above peer average suggests premium valuation relative to growth</li>')
