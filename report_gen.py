...
class ReportGenerator:
    ...
    def _get_llm_content(self, prompt):
        ...
        try:
            print("Requesting content from GPT-4o...")
            response = self.client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "Senior Equity Research Director. Provide a Rigorous, Professional Analysis. For each section, write multiple paragraphs of deep insight (around 300 words per section). Bullet points only for specific metrics. Use headers: 1. EXEC, 2. FINANCIALS, 3. VALUATION, 4. STRATEGIC, 5. CONCLUSION."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=2500,
                temperature=0.7
            )
            print("Content received from LLM.")
            return response.choices[0].message.content
        except Exception as e:
            print(f"LLM Synthesis Error: {e}")
            return f"Analysis summary: Numerical growth remains strong. [Error: {e}]"
    ...
    def _get_section(self, text, header_num, next_header=None):
        # Improved regex to handle various markdown and plain text formats
        # Matches "1. EXEC", "## 1. EXEC", "**1. EXEC**", etc.
        pattern = rf"(?:^|\n)(?:[#*\s]*)?{header_num}\.\s*.*?(?:\n|$)"
        ...
        print(f"Warning: Section {header_num} not found. Returning full text as fallback.")
        return text.strip()[:1000] + "..." if header_num == 1 else "Analysis details in summary."
