...
from src.utils.helpers import get_latest, get_historical, safe_divide
...
class FinancialRatiosCalculator:
    ...
    def _get_historical(self, data_list: List[Dict], field: str, years: int = 5) -> List[float]:
        return get_historical(data_list, field, years)
    ...
    def get_dupont_analysis(self) -> Dict:
        """
        Get 3-Factor DuPont analysis with interpretation.

        ROE = Net Margin × Asset Turnover × Equity Multiplier

        Returns:
            Dict containing 3-factor analysis with insights
        """
        dupont = self.calculate_dupont_3_factor()

        primary_driver = None
        driver_explanation = None

        if dupont['net_margin'] and dupont['asset_turnover'] and dupont['equity_multiplier']:
        ...
        return {
            ...
            'asset_turnover': dupont['asset_turnover'],
            'equity_multiplier': dupont['equity_multiplier'],
            'calculated_roe': dupont['calculated_roe'],
            'actual_roe': dupont['actual_roe'],
