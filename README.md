...
5. **Visualisation**
   Visual outputs such as equity curves and drawdown charts are generated to support interpretation of strategy behaviour and portfolio-level risk dynamics.
...
These features are combined to generate **systematic long/flat trading signals**. The emphasis is placed on **clarity, reproducibility, and financial logic**, rather than indicator novelty or parameter optimisation.
...
- Transaction cost-adjusted results

Backtesting is used to assess whether technical signals generate **interpretable and economically meaningful insights** that could inform portfolio-level timing or risk overlays.
...
|----------|---------|-------------|
| `--ticker` | **Required** | Stock ticker symbol (e.g., NVDA, AAPL, MSFT) |
| `--period` | 10y | Historical data period |
| `--outdir` | outputs | Output directory |
| `--no-llm` | False | Disable LLM report generation |
