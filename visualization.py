...
def plot_ratio_panels(df_calc: pd.DataFrame) -> list[Path]:
    ...
    for ax, (col, title, is_pct) in zip(axes, metrics_5):
        plot_metric_ax(
            ...
            df=d,
            col=col,
            title=f"NVDA {title} (FY{d.index.min()}–FY{d.index.max()})",
            is_percent=is_pct,
        )
...
def plot_multiples_figures(multiples: pd.DataFrame) -> list[Path]:
    ...
    # -------- NEW: bar P/E --------
    fig = plt.figure()
    ax = multiples["P/E (TTM)"].sort_values().plot(
        kind="bar",
        title=f"P/E (TTM) — as of {CFG.asof}",
        rot=0,
    )
    ax.set_ylabel("x")
    outs.append(save_fig(fig, "multiples_pe_bar.png"))

    # bar EV/EBITDA
    fig = plt.figure()
    ax = multiples["EV/EBITDA (TTM)"].sort_values().plot(
        kind="bar",
        title=f"EV/EBITDA (TTM) — as of {CFG.asof}",
        rot=0,
    )
    ax.set_ylabel("x")
    ...
    ax = multiples.loc[[o for o in order if o in multiples.index], "EV/Sales (TTM)"].plot(
        kind="bar",
        title=f"EV/Sales (TTM) — as of {CFG.asof}",
        rot=0,
    )
    ax.set_ylabel("x")
